---
name: plan-reviewer
description: Use when the user wants Codex to draft, review, annotate, and iterate on a plan before producing the final Plan-mode proposal.
---
...
时，使用本技能。目标是让 Codex 先生成可批注的计划草案，通过本地审阅面板收集用户意见，
再根据批注重写草案，直到用户通过为止。
...
- 用户提交回答、提交批注或通过计划后，Codex 必须主动等待并读取 Session 结果，继续回写下一步。
...
3. 需求信息不足时，调用 `publish_requirement_clarification_to_session`。每次只问一个问题，多选问题最多给 A/B/C 三个方案，并设置一个 `recommended: true` 的推荐项和 `recommendationReason`；面板会提供 D: 其他输入。
...
3. 如果需要继续澄清，调用 `publish_requirement_clarification_to_session({ session_id, question, known_context, options, allow_freeform: true, confidence_target: 95 })`。
...
以下是旧版单轮澄清工具的兜底协议。只有在 Session 工具不可用时才使用。
...
以下是旧版单轮计划审阅工具的兜底协议。Session 工具可用时，应优先使用
...
3. 如果 `plan_reviewer` enabled 但工具仍不可见，使用 shell 兜底模式。不要临时编写 Python `importlib` 代码导入插件模块，也不要硬编码 `.codex/plugins/cache/.../<version>` 目录；直接通过 `codex plugin list` 定位当前插件路径并调用脚本入口：

```bash
cd "$(codex plugin list | awk '/^plan-reviewer@personal/ {print $4; exit}')"
...
状态文件使用 `reviews.lock` 做跨进程写保护，避免 MCP、CLI 兜底和旧面板进程同时写入时互相覆盖。
